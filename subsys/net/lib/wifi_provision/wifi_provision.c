...
static struct http_req {
	...
	char url[CONFIG_WIFI_PROVISION_URL_MAX_SIZE];
	char body[CONFIG_WIFI_PROVISION_BODY_MAX_SIZE];
} request;
...
static void finished_entry(void *o)
{
	...
	/* SoftAP mode disabled, unblock wifi_provision_start(). */
...
static int handle_http_request(struct http_req *request)
{
	...
	if ((strlen(request->url) == sizeof("/prov/networks") - 1) &&
		...
		ret = parse_and_store_credentials(request->body, strlen(request->body));
		if (ret) {
