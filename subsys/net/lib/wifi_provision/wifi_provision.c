...
static uint8_t scan_result_buffer[CONFIG_WIFI_PROVISION_SCAN_RESULT_BUFFER_SIZE];
...
DNS_SD_REGISTER_TCP_SERVICE(wifi_provision_sd, CONFIG_NET_HOSTNAME, "_http", "local",
			    txt_record, CONFIG_WIFI_PROVISION_TCP_PORT);
...
#if defined(CONFIG_WIFI_PROVISION_SERVER_CERTIFICATE_REGISTER)
...
static void handle_wifi_scan_result(struct net_mgmt_event_callback *cb)
{
	...
	if (scan.results_count < ARRAY_SIZE(scan.results)) {
		...
		/* SSID */
		strncpy(scan.results[scan.results_count].wifi.ssid, entry->ssid,
			sizeof(entry->ssid));
		scan.results[scan.results_count].wifi.ssid[sizeof(entry->ssid) - 1] = '\0';
		...
		scan.results[scan.results_count].wifi.authMode =
...
static int ap_enable(void)
{
	...
	/* Combine the NIC specific part of the link address and CONFIG_WIFI_PROVISION_SSID.
	 * This is used to create a unique SSID for the AP so that multiple devices on the same
	 * network can be distinguished.
	 */
	char ssid[NIC_ADDR_SIZE + sizeof(CONFIG_WIFI_PROVISION_SSID)];
	...
	strncat(ssid, CONFIG_WIFI_PROVISION_SSID, sizeof(ssid) - 1);
...
static int parse_and_store_credentials(const char *body, size_t body_len)
{
	...
	enum wifi_security_type sec_type =
		creds.info.authMode == AuthMode_WPA_WPA2_PSK ? WIFI_SECURITY_TYPE_PSK :
		creds.info.authMode == AuthMode_WPA2_PSK ? WIFI_SECURITY_TYPE_PSK_SHA256 :
		creds.info.authMode == AuthMode_WPA3_PSK ? WIFI_SECURITY_TYPE_SAE :
		WIFI_SECURITY_TYPE_NONE;

	/* Preferred band is set using a flag. */
	uint32_t flag = (creds.info.band == Band_BAND_2_4_GHZ) ? WIFI_CREDENTIALS_FLAG_2_4GHz :
			(creds.info.band == Band_BAND_5_GHZ) ? WIFI_CREDENTIALS_FLAG_5GHz : 0;

	ret = wifi_credentials_set_personal(creds.info.ssid, strlen(creds.info.ssid),
					    sec_type, NULL, 0,
					    creds.passphrase, strlen(creds.passphrase),
					    flag, creds.info.channel);
	if (ret) {
...
static int process_tcp4(void)
{
	...
	while (true) {
		...
		if (credentials_stored) {
			...
			if (IS_ENABLED(CONFIG_WIFI_PROVISION_SOCKET_CLOSE_ON_COMPLETION)) {
...
int wifi_provision_start(const wifi_provision_evt_handler_t handler)
{
	 ...
	 * Release semaphore and remove callback handler for Wi-Fi events.
	 */
 ...
 * to this workqueue.
 */
static void wifi_provision_thread_fn(void)
{
	int ret;
...
K_THREAD_DEFINE(wifi_provision_thread_id, CONFIG_WIFI_PROVISION_THREAD_STACK_SIZE,
		wifi_provision_thread_fn, NULL, NULL, NULL, 3, 0, 0);
