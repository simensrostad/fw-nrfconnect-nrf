...
/* Function that obtains either the IPv4 or IPv6 address associated with
...
int ipv6_addr_add(const struct net_if *iface)
{
	...
	ifaddr = net_if_ipv6_addr_add((struct net_if *)iface, &net_sin6(&addr)->sin6_addr, NET_ADDR_MANUAL, 0);
	if (!ifaddr) {
		return -EFAULT;
	}
...
{
	if (!ctx.ipv4_added) {
		LOG_DBG("No IPv4 address removed");
		return 0;
	}
...
{
	if (!ctx.ipv6_added) {
		LOG_DBG("No IPv6 address removed");
		return 0;
	}
