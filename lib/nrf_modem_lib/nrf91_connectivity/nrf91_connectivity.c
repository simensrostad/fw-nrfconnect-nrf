...
#include "nrf91_ip_addr_helper.h"
...
BUILD_ASSERT((CONFIG_NET_CONNECTION_MANAGER_STACK_SIZE >= 1024),
	     "Increase the stack size of the connection manager's internal thread");
...
static uint8_t net_if_down_options = NRF91_CONNECTIVITY_IF_DOWN_MODEM_SHUTDOWN;
...
static void fatal_error_notify_and_disconnect(void)
{
	...
	(void)nrf91_connectivity_disconnect(NULL);
}
...
static void connection_timeout_work_fn(struct k_work *work)
{
	...
	int ret = nrf91_connectivity_disconnect(NULL);

	if (ret) {
		LOG_ERR("nrf91_connectivity_disconnect, error: %d", ret);
		net_mgmt_event_notify(NET_EVENT_CONN_IF_TIMEOUT, iface_bound);

		/* Notify that the connection attempt timed out before returning. */
	}
...
static void on_pdn_activated(void)
{
	...
	ret = nrf91_ipv4_addr_add(iface_bound);
	if (ret == -ENODATA) {
...
static void on_pdn_deactivated(void)
{
	...
	ret = nrf91_ipv4_addr_remove(iface_bound);
	if (ret) {
	...
	ret = nrf91_ipv6_addr_remove(iface_bound);
	if (ret) {
	...
	if (conn_mgr_if_get_flag(iface_bound, CONN_MGR_IF_PERSISTENT)) {
		 ...
		 */
		ret = nrf91_connectivity_disconnect(NULL);
		if (ret) {
...
static void on_pdn_ipv6_up(void)
{
	...
	ret = nrf91_ipv6_addr_add(iface_bound);
	if (ret) {
...
/* Public APIs */
void nrf91_connectivity_init(struct conn_mgr_conn_binding *if_conn)
{
	int ret;
	int timeout = CONFIG_NRF91_CONNECTIVITY_CONNECT_TIMEOUT_SECONDS;
	int persistent = IS_ENABLED(CONFIG_NRF91_CONNECTIVITY_CONNECTION_PERSISTENCE);
	...
	LOG_DBG("Connection persistence is %s", (if_conn->flags & BIT(CONN_MGR_IF_PERSISTENT)) ?
						"enabled" : "disabled");
	...
	LOG_DBG("nRF91 Connectivity library initialized");
}
...
int nrf91_connectivity_disable(void)
{
	...
	if (net_if_down_options == NRF91_CONNECTIVITY_IF_DOWN_MODEM_SHUTDOWN) {
		ret = nrf_modem_lib_shutdown();
		if (ret) {
			LOG_ERR("nrf_modem_lib_shutdown, retval: %d", ret);
			return ret;
		}
	} else if (net_if_down_options == NRF91_CONNECTIVITY_IF_DOWN_LTE_DISCONNECT) {
		ret = nrf91_connectivity_disconnect(NULL);
		if (ret) {
			LOG_ERR("nrf91_connectivity_disconnect, retval: %d", ret);
			return ret;
		}
	} else {
				   ...
				   size_t length)
{
	...
	if (name != NRF91_CONNECTIVITY_IF_DOWN) {
		return -ENOPROTOOPT;
	}
	...
	case NRF91_CONNECTIVITY_IF_DOWN_MODEM_SHUTDOWN:
		LOG_DBG("Shutdown modem when the network interface is brought down");
		break;
	case NRF91_CONNECTIVITY_IF_DOWN_LTE_DISCONNECT:
