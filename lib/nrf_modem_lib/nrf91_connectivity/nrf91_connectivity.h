...
/**
 * @defgroup nrf91_connectivity nRF91 connectivity layer
 * @{
 * @brief Library that provides connectivity for nRF91 based boards.
 *	  The library is intended to be integrated with and bound to a Zephyr network interface.
 ...
 *	   calling net_if_down().
 */
enum nrf91_connectivity_if_down_options {
	/** Shutdown modem - GNSS functionality will not be preserved. */
	NRF91_CONNECTIVITY_IF_DOWN_MODEM_SHUTDOWN = 1,

	/** Disconnect from LTE - GNSS functionality will be preserved. */
	NRF91_CONNECTIVITY_IF_DOWN_LTE_DISCONNECT
};

/** @brief Option types used to specify library behavior. */
enum nrf91_connectivity_options {
	/** Sets desired behavior when nrf91_connectivity_disable() / net_if_down() is called. */
	NRF91_CONNECTIVITY_IF_DOWN = 1,
};

/** @brief Macro used to define a private Connection Manager connectivity context type.
 *         Required but not implemented.
 */
#define NRF91_CONNECTIVITY_CTX_TYPE void *

/** @brief Initialize the nRF91 connectivity layer.
 *
 ...
 *	     - The network interface's auto option NET_IF_NO_AUTO_START
 *	     - Connection timeout and persistency.
 *
...
/** @brief Enable the nRF91 connectivity layer.
 *
...
/** @brief Disable the nRF91 connectivity layer.
 *
 *  @details The behavior of this function changes depending on library options:
 *
 *	     If NRF91_CONNECTIVITY_IF_DOWN_MODEM_SHUTDOWN is set:
 *
 *	     Modem is completely shutdown and no modem functionality is preserved.
 *	     This is useful after modem DFU when the modem needs to be reinitlized which requires
 *	     that the modem is shutdown first. Reinitialization can be carried out by calling
 *	     net_if_up() in the application.
 *
 *	     If NRF91_CONNECTIVITY_IF_DOWN_LTE_DISCONNECT is set:
 *
