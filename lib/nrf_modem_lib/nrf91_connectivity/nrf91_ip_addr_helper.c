...
int nrf91_ipv4_addr_add(const struct net_if *iface)
{
	int len;
...
int nrf91_ipv6_addr_add(const struct net_if *iface)
{
	int len;
...
int nrf91_ipv4_addr_remove(const struct net_if *iface)
{
	if (iface == NULL) {
		return -EINVAL;
	}
...
int nrf91_ipv6_addr_remove(const struct net_if *iface)
{
	if (iface == NULL) {
		return -EINVAL;
	}
