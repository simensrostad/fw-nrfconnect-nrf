...
* For :ref:`lib_wifi_credentials` library and Wi-Fi samples:
  ...
  * :kconfig:option:`CONFIG_WIFI_CREDENTIALS_BACKEND_PSA_UID_OFFSET` has been removed because it was specific to the previous solution that used PSA Protected Storage instead of PSA Internal Protected Storage.
    Use :kconfig:option:`CONFIG_WIFI_CREDENTIALS_BACKEND_PSA_OFFSET` to specify the key offset for PSA Internal Protected Storage.
    Be aware that Wi-Fi credentials stored in Protected Storage will not appear in Internal Protected Storage when switching.
    To avoid re-provisioning Wi-Fi credentials, manually read out the old credentials from Protected Storage in the previously used UID and store to Internal Protected Storage.
